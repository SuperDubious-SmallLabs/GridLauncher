...
void addToolbarButton(int buttonType, void (*callback)(void*, void*), int tag) {
    button aButton;
...
void toolbarTopLeftAction(void* obj1, void* obj2) {
    handleMenuTopLeftActions(menuTopLeftActionSourceTopLeft);
}

void toolbarTopRightAction(void* obj1, void* obj2) {
...
void toolbarBottomLeftAction(void* obj1, void* obj2) {
    bool playSelectSound = true;
...
void toolbarBottomRightAction(void* obj1, void* obj2) {
    bool playSelectSound = true;
...
void drawGrid(menu_s* m) {
    if (transitionFromPage > -1) {
        ...
        float t = (float)transitionStep / (float)transitionTotalSteps;
        if (t > 1.0f) t = 1.0f;

        float u = 1.0f - t;
        float eased = 1.0f - u*u*u*u*u;

        transitionInPixel = (int)((float)transitionDistance * (1.0f - eased));
        transitionOutPixel = (int)((float)(-transitionDistance) * eased);
...
void startTransition(int direction, int fromPage, menu_s* fromMenu) {
    ...
    transitionInPixel = transitionDistance;
    transitionOutPixel = 0;
    transitionStep = 0;
    transitionTotalSteps = 18;
...
void handleDPadToolbarActions(menu_s* m) {
    if (dPadSelectedToolbarButton == 0) {
        toolbarTopLeftAction(NULL, NULL);
    }
    else if (dPadSelectedToolbarButton == 1) {
        toolbarTopRightAction(NULL, NULL);
    }
    else if (dPadSelectedToolbarButton == 2) {
        toolbarBottomRightAction(NULL, NULL);
    }
    else if (dPadSelectedToolbarButton == 3) {
        toolbarBottomLeftAction(NULL, NULL);
    }
}
...
//        if(1) {
            ...
            if (me->isRegionFreeEntry && regionFreeGamecardIn) {
                ...
                if (me->isRegionFreeEntry) {
                    char * insert = "Insert a game cart";
