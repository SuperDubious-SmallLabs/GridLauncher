...
menu_s settingsMenu = {0};
menu_s themesMenu = {0};
menu_s waterMenu = {0};
menu_s themeSettingsMenu = {0};
menu_s gridSettingsMenu = {0};
...
void randomiseTheme() {
    ...
    if (randomEntry && randomEntry->executablePath[0] != '\0') {
...
void addSettingsMenuEntry(char * name, char * description, u8 * icon, bool * showTick, menu_s *m,  void (*callback)(void*, void*), void *callbackObject1, void *callbackObject2) {
...
void settingsToggleBool(void* obj1, void* obj2) {
    bool* aBool = (bool*)obj1;
    *aBool = !(*aBool);
}

void settingsToggleRegionFree(void* obj1, void* obj2) {
    showRegionFree = !showRegionFree;
    menuRegionFreeToggled();
}

void settingsToggleShow3DS(void* obj1, void* obj2) {
...
void settingsToggleSortAlpha(void* obj1, void* obj2) {
    sortAlpha = !sortAlpha;
    menuReloadRequired = true;
}

void settingsShowColours(void* obj1, void* obj2) {
...
void settingsSetMenuStatus(void* obj1, void* obj2);
...
void cycleRows(void* obj1, void* obj2) {
    int maxRows = 3;
    int minRows = 1;
...
void cycleLogoType(void* obj1, void* obj2) {
    logoType++;
...
void startUpdate(void* obj1, void* obj2) {
...
void initConfigMenu() {
    ...
    if (regionFreeAvailable) {
        addSettingsMenuEntry("Launch Cartridge", "Toggle the Launch Cartridge entry in the main menu grid", (u8*)regionfreeEntry.iconData, &showRegionFree, &settingsMenu, &settingsToggleRegionFree, NULL, NULL);
    }
    ...
    if (fileExists("/3ds/gridlauncher/update/mglupdate.3dsx", &sdmcArchive) && fileExists("/3ds/gridlauncher/update/index.lua", &sdmcArchive)) {
...
static char * configArchivePathForType(int configType) {
    if (configType == configTypeMain) {
        ...
        char * path = malloc(strlen(themePath) + strlen("hbltheme.cfg") + 1);
        if (!path) { free(themePath); return NULL; }
        sprintf(path, "%shbltheme.cfg", themePath);
        free(themePath);
        return path;
    }
    return NULL;
...
void loadConfigWithType(int configType) {
    ...
    char * configPath = configArchivePathForType(configType);
    if (!configPath) return;

    if (!fileExists(configPath, &sdmcArchive)) {
        free(configPath);
        data->numConfigEntries = 0;
        return;
    }

    Handle fileHandle;
    Result ret = FSUSER_OpenFile(&fileHandle, sdmcArchive,
                                  fsMakePath(PATH_ASCII, configPath),
                                  FS_OPEN_READ, 0);
    free(configPath);
    if (R_FAILED(ret)) {
        data->numConfigEntries = 0;
    ...
    static char configText[8192];
    u32 toRead = (u32)fileSize;
    if (toRead >= sizeof(configText)) toRead = sizeof(configText) - 1;

    u32 bytesRead = 0;
    ret = FSFILE_Read(fileHandle, &bytesRead, 0, configText, toRead);
    FSFILE_Close(fileHandle);

    if (R_FAILED(ret) || bytesRead == 0) {
        data->numConfigEntries = 0;
        return;
    }
    ...
    char * split1 = strtok(configText, "|");
    while (split1 != NULL && count < 64) {
        strncpy(lines[count], split1, 63);
        lines[count][63] = '\0';
        count++;
        split1 = strtok(NULL, "|");
    }
    ...
    int i;
    for (i = 0; i < count; i++) {
        char * split2 = strtok(lines[i], "=");
        if (!split2) continue;

        if (strcmp(split2, "translucencyLevel") == 0) {
            count--;
        } else {
            strncpy(data->keys[i], split2, 63);
            data->keys[i][63] = '\0';
            split2 = strtok(NULL, "=");
            if (split2) {
                strncpy(data->values[i], split2, 63);
                data->values[i][63] = '\0';
            }
        }
    }
...
void saveConfigWithType(int configType) {
    configData *data = configDataForType(configType);
    if (!data || data->numConfigEntries == 0) return;

    int size = 1;
    int i;
    for (i = 0; i < data->numConfigEntries; i++) {
        size += strlen(data->keys[i]) + strlen(data->values[i]) + 2;
    }
    ...
    for (i = 0; i < data->numConfigEntries; i++) {
        strcat(out, data->keys[i]);
        strcat(out, "=");
        strcat(out, data->values[i]);
        strcat(out, "|");
    }

    u32 outLen = strlen(out);

    char * configPath = configArchivePathForType(configType);
    if (!configPath) { free(out); return; }

    FSUSER_DeleteFile(sdmcArchive, fsMakePath(PATH_ASCII, configPath));

    Result ret = FSUSER_CreateFile(sdmcArchive, fsMakePath(PATH_ASCII, configPath),
                                    0, outLen);
    if (R_FAILED(ret)) { free(out); free(configPath); return; }

    Handle fileHandle;
    ret = FSUSER_OpenFile(&fileHandle, sdmcArchive,
                           fsMakePath(PATH_ASCII, configPath),
                           FS_OPEN_WRITE, 0);
    free(configPath);
    if (R_FAILED(ret)) { free(out); return; }

    u32 bytesWritten = 0;
    FSFILE_Write(fileHandle, &bytesWritten, 0, out, outLen, FS_WRITE_FLUSH);
    FSFILE_Close(fileHandle);
